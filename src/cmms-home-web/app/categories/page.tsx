...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    api.categories.list().then(setItems).finally(() => setLoading(false))
  }, [])
  ...
  async function handleAdd() {
    ...
    setAdding(true)
    try {
      const item = await api.categories.create(name)
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-4">
      ...
      {!loading && items.length === 0 && (
        <p className="text-slate-400 dark:text-slate-500 text-sm text-center py-8">No categories yet.</p>
      )}
